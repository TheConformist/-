...
string File_des_path = "C:\\Users\\xumin\\Desktop\\2022大三上学期\\计算机网络\\lab3\\测试结果3\\";
...
void recvdata()
{
    ...
    while (true)
    {
        ...
        if (checkerror((u_short*)&recv_header, sizeof(recv_header)) == 0 && expectedseqnum == recv_header.seq)
        {
            ...
            //int ack = recv_seq + 1;
            response.ack = expectedseqnum + 1; //希望收到的下一个报文序列号，这次收到22，希望下次收到23
        ...
        else  //报文传输有错误，需要重传
        {
            ...
            int recv_seq = recv_header.seq;
            //int ack = recv_seq + 1;
            response.ack = expectedseqnum; //希望收到报文22！
