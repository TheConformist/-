...
bool one_recv()
{
	...
	{
		//收到新的ACK
		if (base < recvpacketheader.ack)
		{
			if (state == 1)//慢启动阶段
			{
				cout << "【慢启动阶段】接收new ACK" << endl;
				base = recvpacketheader.ack; //base挪动
				cout << "base挪动为" << base << endl;
				RTTcount = 0;
				dupACKcount = 0;
				cwnd += 1;
				cout << "窗口大小加1，当前窗口大小为：" << cwnd << endl;
				onepacket_start = clock();
				return 1;
			}
			else if (state == 2) //拥塞避免阶段
			{
				cout << "【拥塞避免阶段】接收new ACK" << endl;
				if (RTTcount < cwnd)
				{
					base = recvpacketheader.ack; //base挪动
					cout << "base更新为" << base << endl;
					dupACKcount = 0;
					RTTcount += recvpacketheader.ack - base;//这期间增加的报文的个数
					//当这段时间内收到的报文数目达到窗口值时，即达到一个RTT
					if (RTTcount >= cwnd)
					{
						RTTcount -= cwnd;
						cwnd += 1;
					...
					//cout << "RTTcount大小为" << RTTcount << endl;
	...
	//收到冗余ACK
	//else if (base == recvpacketheader.ack)
	else if (recvpacketheader.flag_ack==0)//NAK，乱序报文情形，由于基础还是GBN，ack等于base
	{
		if (state == 1)
			cout << "【慢启动阶段】";
		else if(state==2)
			...
			cout << "【快速恢复阶段】";
		cout << "收到重复ACK" << recvpacketheader.ack << endl;

		//快速恢复阶段,对应于状态图中快速恢复自己指向自己的箭头
		if (state == 3)
		{
			cwnd += 1;
			cout << "窗口大小加1，当前窗口大小为：" << cwnd << endl;
		...
		{
			dupACKcount += 1;
			cout << "重复ACK数量加1，当前重复ACK数目：" << dupACKcount << endl;
			////3次重复ACK进入快速恢复，就是慢启动和拥塞避免指向快速恢复的箭头
			if (dupACKcount == 3)
			{
				state = 3;
				ssthresh = cwnd / 2;
				...
				cout << "进入【快速恢复阶段】" << endl;
				cout << "Reno，重传对应报文" << endl;
				//Reno里，只传一个数据包就行
				char* resendpacket1 = new char[MTU];//分包发送
				MyPacketHeader resendpakcetheader1;
				memcpy(resendpacket1, &resendpakcetheader1, sizeof(resendpakcetheader1));
				int packetsize = min(filesize - base * (MTU - 10), MTU - 10);
				resendpakcetheader1.datalength = packetsize;
				for (int j = 0;j < packetsize;j++)
				{
					resendpacket1[j + 10] = TheFile[j + base * (MTU - 10)];
				}
				resendpakcetheader1.seq = base;
				resendpakcetheader1.ack = 1;
				if (base == packetnum - 1)//如果发了最后一个包
					resendpakcetheader1.flag = 4;
				resendpakcetheader1.verification = checkerror((u_short*)&resendpakcetheader1, sizeof(resendpakcetheader1));//校验和
				memcpy(resendpacket1, &resendpakcetheader1, sizeof(resendpakcetheader1));
				sendto(socket_sender, resendpacket1, MTU, 0, (sockaddr*)&receiver_address, receiver_address_len);
				cout << "重传序列号为" << base << "的报文" << endl;
...
void mytimechecker(void*)
{
	while (base != packetnum)
	{
		...
		if (onepacket_end - onepacket_start >= MAX_TIME)
		{
			...
			cout << "窗口大小更新为" << cwnd << " 阈值更新为" << ssthresh << endl;
			cout << "进入【慢启动阶段】，开始重传" << endl;
			onepacket_start = clock();
...
void send(string filename)
{
	...
	cout << "当前处于【慢启动阶段】" << endl;

	while (base != packetnum)
	{
		//这是rdt_send(data)，发送nextseqnum处的分组，对应图3-20的最上面那种情况
		//不同于上一实验，这里的窗口大小window改为了cwnd
		//先决条件，base与nextseqnum在窗口以内，nextseqnum没有超过总包数
		if (nextseqnum < base + cwnd && nextseqnum < packetnum)
		{
